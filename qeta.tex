...
\item[qetasamba] Implements the \algoSamba{} algorithm as
  ...
  \begin{itemize}
  \item \code{QEtaComputation1} Since the computation of the set $S$
  ...
  \item \code{QEtaComputation2} Critical elements are pairs of basis
  ...
  \item \code{QEtaExtendedComputation} The behaviour is basically identical
    to \code{QEtaComputation2}. However, the representation is such
    that the reduction steps are recorded, \ie,
    \code{algebra Basis} (\algoSamba) computes a basis together with
